...
cand <- cand %>% count(Age,Respondent) %>% group_by(Age) %>% mutate(pct=n/sum(n))
g <- ggplot(cand, aes(Age, n, fill=Respondent))
#png("age_online.png", width=600, height=600, res=135)
pdf("/Users/tzupingliu/Desktop/Projects/Conjoin and Cabinet/Figure_New/age_online.pdf")
#pdf("age_online.pdf")
