...
cand <- cand %>% count(Education,Respondent) %>% group_by(Education) %>% mutate(pct=n/sum(n))
g <- ggplot(cand, aes(Education, n, fill=Respondent))
#png("education_online.png", width=600, height=600, res=135)
pdf("/Users/tzupingliu/Desktop/Projects/Conjoin and Cabinet/Figure_New/education_online.pdf")
