...
cand$PartyID <- plyr::mapvalues(cand$PartyID, unique(cand$PartyID), c("Conservative", "Labour", "No", "Green", "LDP", "UKIP"))
cand <- cand %>% count(PartyID,Respondent) %>% group_by(PartyID) %>% mutate(pct=n/sum(n))
g <- ggplot(cand, aes(PartyID, n, fill=Respondent))
#png("partyid_online.png", width=600, height=600, res=135)
pdf("/Users/tzupingliu/Desktop/Projects/Conjoin and Cabinet/Figure_New/partyid_online.pdf")
